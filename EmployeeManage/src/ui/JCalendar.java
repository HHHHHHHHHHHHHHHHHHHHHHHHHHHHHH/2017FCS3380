...
public class JCalendar extends JPanel {
	...
	public void setScheduleDao(JobScheduleDao scheduleDao) {
...
//					String text=JOptionPane.showInputDialog("请输入任务内容");
//					if (!text.isEmpty()) {
//						days[index].setForeground(Color.RED);
//					}
//					showDate();
//					showDays();
//				}
//			});
			days[i].addMouseListener(new MouseAdapter() {
				public void mouseClicked(MouseEvent e){
					if (parent.selectEmp==null) {
						JOptionPane.showMessageDialog(JCalendar.this, "Please select the staff!");
						return;
					}
					day = Integer.parseInt(((JToggleButton) e.getSource()).getText());
					String date=Integer.toString(year) + "-" + Integer.toString(month + 1) + "-" + Integer.toString(day);
					if (e.getButton()==MouseEvent.BUTTON1) {
						String text=JOptionPane.showInputDialog("Please input the content of job!");
						if (text!=null&&!text.isEmpty()) {
							days[index].setForeground(Color.RED);
							try {
								parent.addJob(text, new SimpleDateFormat("yyyy-MM-dd").parse(date));
						...
						try {
							JobSchedule jobSchedule=scheduleDao.findJob(parent.selectEmp.getId(), new SimpleDateFormat("yyyy-MM-dd").parse(date));
							if (jobSchedule==null) {
								return;
							}
							JOptionPane.showMessageDialog(JCalendar.this, jobSchedule.getEvent());
						} catch (ParseException e1) {
							// TODO Auto-generated catch block
							e1.printStackTrace();
						}
	...
	public void setEmp(int empId){
		List<JobSchedule> jobs=scheduleDao.getJobs(empId);
		for ( JToggleButton day : days) {
			day.setForeground(Color.BLACK);
		}
		for (JobSchedule jobSchedule : jobs) {
			Date date=jobSchedule.getDate();
			if (date.getMonth()==month) {
				for ( JToggleButton day : days) {
					if (String.valueOf(date.getDate()).equals(day.getText())) {
